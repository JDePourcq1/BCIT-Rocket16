...
/**
 * sets up the traveller selection based on which achievements have been unlocked
 */
var setupUnlockedTravellers = function() {
    $.ajax({
        async: false,
        type: 'GET',
        url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + currentUser + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
        dataType: 'json',
        success: function(data) {
            for (i = 1; i <= 6; i++) {
                if (i == 6) {
                    if (data[0].achievements["easter"]) {
                        $('#item' + i + 'Lock').attr('style', 'display: none;');
                        $('#C'+i).attr('style', 'display: none;');
                        $('#item'+i).attr('style', 'display: block;');
                    } else {
                        $('#item' + i + 'Lock').attr('style', 'display: block;');
                        $('#C'+i).attr('style', 'display: block;');
                        $('#item'+i).attr('style', 'display: none;');
                    }
                } else if (data[0].achievements[i.toString()]) {
                    $('#item' + i + 'Lock').attr('style', 'display: none;');
                    $('#C'+i).attr('style', 'display: none;');
                    $('#item'+i).attr('style', 'display: block;');
                } else {
...
/**
 * gets the user's current traveller label as a string
 * @param user - the username
 * @return the users current traveller icon
 */
...
/**
 * Sets the user's traveller icon
 * @param user - the username
 * @param newIcon - the new icon as a string
 */
var setTravellerIcon = function(newIcon) {
    $.ajax({ 
        url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + currentUser + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
    ...
    alert("Traveler Icon changed");
}

/**
 * checks if the score the user got is higher then the minimum
 * @param crsNo - the course 
 * @param mins - the minutes
 * @param secs - the seconds
 * @param mils - the tenths of milliseconds
 * @return true/false - whever its greater than the min pass time or not
 */
var checkScore = function(crsNo, mins, secs, mils) {
    var good = false;
    $.ajax({
        async: false,
        type: 'GET', 
        ...
        dataType: 'json',
        success: function (data) {
            if (parseInt(mins) < parseInt(data[0].minTime.min)) {
                good = true;
            } else if (parseInt(mins) == parseInt(data[0].minTime.min) && parseInt(secs) <= parseInt(data[0].minTime.sec)) {
                good = true;
            }
        }
    });
 ...
 */
 var unlockAchievement = function(user, achievement) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function (data) {
            console.log(data[0].achievements[achievement]);
            if (data[0].achievements[achievement] == false) {
                alert("You unlocked a new achievement!!!");
                switch (achievement) {
                    case "easter":
                        $.ajax({ 
                            url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + user + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
                            async: false,
                            data: JSON.stringify( { "$set" : { "achievements.easter" :  true} } ),
                            type: "PUT",
                            contentType: "application/json" 
                        });
                        break;
                    case "1":
                        $.ajax({ 
                            url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + user + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
                            async: false,
                            data: JSON.stringify( { "$set" : { "achievements.1" :  true} } ),
                            type: "PUT",
                            contentType: "application/json" 
                        });
                        break;
                    case "2":
                        $.ajax({ 
                            url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + user + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
                            async: false,
                            data: JSON.stringify( { "$set" : { "achievements.2" :  true} } ),
                            type: "PUT",
                            contentType: "application/json" 
                        });
                        break;
                    case "3":
                        $.ajax({ 
                            url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + user + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
                            async: false,
                            data: JSON.stringify( { "$set" : { "achievements.3" :  true} } ),
                            type: "PUT",
                            contentType: "application/json" 
                        });
                        break;
                    case "4":
                        $.ajax({ 
                            url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + user + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
                            async: false,
                            data: JSON.stringify( { "$set" : { "achievements.4" :  true} } ),
                            type: "PUT",
                            contentType: "application/json" 
                        });
                        break;
                    case "5":
                        $.ajax({ 
                            url: 'https://api.mlab.com/api/1/databases/kitty_cat_database/collections/User?q={"username": "' + user + '"}&apiKey=AMVdmMQFjEHmgWz3ppXtWHqCaOaBm2XB',
                            async: false,
                            data: JSON.stringify( { "$set" : { "achievements.5" :  true} } ),
                            type: "PUT",
                            contentType: "application/json" 
                        });
                        break;
        ...
        success: function (data) {
            $.ajax({ 
                ...
                data: JSON.stringify( { "scoreID": data.toString(),
                                        ...
                                        "score": {
                                            "min": mins.toString(),
                                            "sec": secs<10?"0"+secs:secs.toString(),
                                            "mil": mils.toString()
                                        }} ),
                type: "POST",
                contentType: "application/json" 
...
var getCourseScores = function () {	
    for (i = 0; i < 5; i++) {
        $.ajax({
            ...
            success: function(data) {
                if (data.length > 0) {
                    document.getElementById('course' + (i+1) + 'Best').innerHTML = data[0].score.min + ":" + data[0].score.sec + "." + data[0].score.mil;
                } else {
