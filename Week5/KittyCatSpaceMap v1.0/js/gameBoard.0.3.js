...
function Traveller(posPoint, nodeId) {
    var curIcon = getTravellerIcon(currentUser);
	this.icon = new paper.Raster({
        
		source: travellerIcon[curIcon],
		position: posPoint,
		data: {
			id: nodeId
		}
...
var goToResults = function() {
    ...
    if(currentUser != "") {
        ...
        if (name != "" && name != null) {
            postScore(name, currentCourse, min, sec, mil);
        } else {
            postScore("guest", currentCourse, min, sec, mil);
        }
    }
    if(checkScore(currentCourse, min, sec, mil)) {
        if (currentUser != "") {
            unlockAchievement(currentUser, parseInt(currentCourse)+1);
        }
    } else {
        alert("Didnt beat par!");
    }
